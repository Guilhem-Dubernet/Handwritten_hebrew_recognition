...
# define labels and prediction model
loaded_model = tf.keras.models.load_model('./resnet_model_softmax')

labels_index = {'aleph': 5, 'ayin': 13, 'beit': 9, 'dalet': 21, 'guimel': 16, 'he': 23, 'het': 2, 'khaf': 4, 'khaf_final': 6, 'lamed': 3, 'mem': 1, 'mem_final': 17, 'noun': 14, 'noun_final': 11, 'pe': 24, 'pe_final': 25, 'qof': 19, 'resh': 7, 'samekh': 26, 'shin': 18, 'tav': 12, 'tet': 10, 'tsade': 15, 'tsade_final': 8, 'vav': 0, 'yod': 22, 'zayin': 20}

letters_dict = {'vav':'ו', 'mem':'מ', 'het':'ח', 'lamed':'ל', 'khaf':'כ', 'aleph':'א', 'khaf_final':'ך', 'resh':'ר', 'tsade_final':'ץ', 'beit':'ב', 'tet':'ט', 'noun_final':'ן', 'tav':'ת', 'ayin':'ע', 'noun':'נ', 'tsade':'צ', 'guimel':'ג', 'mem_final':'ם', 'shin':'ש', 'qof':'ק', 'zayin':'ז', 'dalet':'ד', 'yod':'י', 'he':'ה', 'pe':'פ', 'pe_final':'ף', 'samekh':'ס'}
...
#Tempting to adapt recognize plate for AI Guy to our model
def recognize_plate(path, coords):

    # separate coordinates from box
    xmin, ymin, xmax, ymax = coords

    xmin = np.int(xmin.item())
    ymin = np.int(ymin.item())
    xmax = np.int(xmax.item())
    ymax = np.int(ymax.item())
    ...
    for cnt in sorted_contours:
        ...
        roi = roi /255.0
        roi = tf.reshape(roi, (1,224,224,3))



        letter = loaded_model.predict(roi)
        letter = np.argmax(letter)
        for g in labels_index.keys():
            if labels_index[g] == letter:
                hbr_unicode = g
        letter = letters_dict[hbr_unicode]
